...
MongoClient.connect(url, function(err, db) {
  app.get('/user/:userid', function(req, res,next) {
    ...
    if (fromUser === userid){
      ...
      console.log("ui: " + userid + " fu: " + fromUser);
    }
    next();
  });
  ...
  function getUserData(user,callback,innercall) {
    // console.log("test");
    ...
    db.collection('users').findOne({
      ...
      if(innercall){
        innercall(userData,callback)
      }else{
        callback(userData);
      }
      //getUserItem(userData, callback);
    });
  }
...
/* function getFeedback(length){
  ...
  app.get('/user/:userid', function(req, res) {
    ...
    if (fromUser === userid) {
      userid = new ObjectID(userid);
        ...
        // console.log("test");
        //getUserData(new ObjectID(userid),genericRouteCallback(res,userid),
          //function(user,callback){getUserItem(user, callback) });
          getUserItem(userid,genericRouteCallback(res,userid));
      } else {
  ...
  */
  ...
  function genericRouteCallback(res,userid){
    return function(err,data){
      //console.log("generic data: " + JSON.stringify(data));
      if (err) {
        // A database internal error 500 happened.
        res.status(500).send("Database error: " + err);
      } else if (data === null) {
        // Couldn't find data in the database.
  ...
  function genericFunctCallback(callback,content) {
    return function(err,data){
  ...
  // Get savePage data
  function getPageData(user,callback,content){
    var collection = "savePage";
    //getUserData(user,function(user){
        db.collection(collection).findOne({_id: user},
          genericFunctCallback(callback));
        //}
    //content.pop();
    //var userData = readDocument('users', user);
  ...
  app.get('/user/:userid/page',function(req,res) {
    ...
    if(fromUser === userid){
      ...
      //there is only one callback, the route callback, that gets passed to everything.
      //where does the content go? a third param.
      /*var contray = [];
      contray.push(content(callback(user)));
      contray.push(content())*/
      getPageData(
        //param 1
        new ObjectID(userid),
          //param 2
          genericRouteCallback(res,userid)
            //param3
              );
    }
    else {
