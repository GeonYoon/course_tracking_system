...
MongoClient.connect(url, function(err, db) {
  ...
  app.all('/user/:userid/*', function (req, res, next) {
    var userid = req.params.userid;
    var fromUser = getUserIdFromToken(req.get('Authorization'));
    var useridNumber = parseInt(userid,10);
    console.log("count: " + count);
    count +=1;
    console.log("from user: " + fromUser + " userid " + userid + " idnum " + useridNumber);
...
/*// function getCourseItemSync(courseId){
  ...
  //   return majorItem;
  // }*/
  //use for route callbacks
  function genericRouteCallback(err, data){
  ...
  //use in functions
  function genericFunctCallback(err, data) {
    //return function(err,data){
      if (err) {
        return callback(err);
      } else if (data === null) {
  ...
  function dataFunctCallback(err, data, callback) {
    ...
    // Get savePage data
  function getPageData(input,callback){
    var collection = 'savePage';
    console.log("input: " + input);
    var userData = getUserData(input,
      function (err, data) {
    ...
    db.collection(collection).findOne({_id: new ObjectID(userData)},
    function (err, data, callback) {
      console.log("tracing3");
      return function(err,data,callback){
        if (err) {
          return callback(err);
        } else if (data === null) {
